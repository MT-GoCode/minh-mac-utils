...
public struct DelayQueue {
    public struct Item: Codable, Equatable {
        ...
        public var applyAt: Double
        public var seq: UInt64
        public var retries: UInt32? = nil        // .retry bookkeeping (optional ⇒ decodeIfPresent — a
        public var nextRetryAt: Double? = nil    //  non-optional default would THROW on older JSON and
                                          //  silently empty the queue; nil means 0)
        public init(payload: String, requestedAt: Double, applyAt: Double, seq: UInt64,
                    retries: UInt32? = nil, nextRetryAt: Double? = nil) {
            self.payload = payload; self.requestedAt = requestedAt; self.applyAt = applyAt; self.seq = seq
            self.retries = retries; self.nextRetryAt = nextRetryAt
        }
    }
    public struct Outcome: Codable, Equatable {
        ...
        public init(key: String, what: String, reason: String?, at: Double) {
            self.key = key; self.what = what; self.reason = reason; self.at = at
        }
    }
    public struct QState: Codable {
        public var pending: [String: Item] = [:]
        public var nextSeq: UInt64 = 0
        public var lastAppliedAt: Double? = nil
        public var recent: [Outcome] = []        // last 8 EVENTS, newest first (batch/flush = one event)
        public init(pending: [String: Item] = [:], nextSeq: UInt64 = 0, lastAppliedAt: Double? = nil, recent: [Outcome] = []) {
            self.pending = pending; self.nextSeq = nextSeq; self.lastAppliedAt = lastAppliedAt; self.recent = recent
        }
        ...
        public mutating func fixSeq() { if let m = pending.values.map(\.seq).max(), nextSeq <= m { nextSeq = m + 1 } }
    }
    public struct Row: Codable {
        public var key: String; public var preview: String; public var applyAt: Double; public var seq: UInt64
        public init(key: String, preview: String, applyAt: Double, seq: UInt64) {
            self.key = key; self.preview = preview; self.applyAt = applyAt; self.seq = seq
        }
    }
    public struct QStatus: Codable {
        public var kind: String
        public var rows: [Row] = []              // seq order
        public var lastAppliedAt: Double? = nil
        public var recent: [Outcome] = []
        public var full: Bool = false
        public init(kind: String, rows: [Row] = [], lastAppliedAt: Double? = nil, recent: [Outcome] = [], full: Bool = false) {
            self.kind = kind; self.rows = rows; self.lastAppliedAt = lastAppliedAt; self.recent = recent; self.full = full
        }
    }
    ...
    public struct QStateStore {
        public let load: () -> QState
        public let save: (QState) -> Void
        public init(load: @escaping () -> QState, save: @escaping (QState) -> Void) { self.load = load; self.save = save }
        ...
        public static func file(_ path: String,
    ...
    public enum Failure { case drop, retry }

    public let kind: String
    public let store: QStateStore
    public let requestMarker: String
    public let abortMarker: String
    public let onFailure: Failure
    public let payloadIsJSON: Bool               // canonicalisation mode — declared, never sniffed
    public let auditLog: String
    public init(kind: String, store: QStateStore, requestMarker: String, abortMarker: String,
                onFailure: Failure, payloadIsJSON: Bool, auditLog: String) {
        self.kind = kind; self.store = store; self.requestMarker = requestMarker; self.abortMarker = abortMarker
        self.onFailure = onFailure; self.payloadIsJSON = payloadIsJSON; self.auditLog = auditLog
    }
    ...
    public static let maxLinesPerMarker = 256
    ...
    public func canonical(_ payload: String) -> String {
    ...
    @discardableResult
    public func consumeMarkers(now: Double, enforcedUID: uid_t?,
    ...
    /// audit trail.
    public func rootCancel(keys: [String], now: Double, reason: String) {
        var st = store.load()
        var hit = false
    ...
    public func peekDue(now: Double) -> [(key: String, payload: String)] {
        store.load().pending
    ...
    public func applyDue(now: Double,
    ...
    @discardableResult
    public func tick(now: Double, enforcedUID: uid_t?,
              delaySec: (String) -> Double,
              key: (String) -> String?,
              validate: (String) -> Bool,
