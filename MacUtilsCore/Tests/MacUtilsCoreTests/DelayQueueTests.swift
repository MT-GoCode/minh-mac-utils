...
final class DelayQueueTests: XCTestCase {
    ...
    // MARK: recent ring + expedite + audit

    func testRecentIsEightEvents_expediteIsOne() {
        let q = queue()
        ...
        XCTAssertEqual(q.expediteAll(now: 1001, reason: "admin grant"), 20)
        let recent = q.status().recent
        XCTAssertEqual(recent.first?.what, "expedited")        // ONE event for 20 rows
        XCTAssertEqual(recent.count, 8)
        XCTAssertTrue(recent.first!.key.contains(","))         // lists the keys
        XCTAssertEqual(q.status().rows.count, 20)              // still pending — due now, not gone
    }
