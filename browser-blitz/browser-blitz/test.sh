...
sec "A. lifecycle"
A=$(new a)
is  "new-session prints slug and CDP url, not internals" "http://127.0.0.1:9342/$A" "$(bb new-session "$A" 2>&1; row "$A")"
is  "list shows it LIVE"                                 "LIVE"                     "$(row "$A")"
isnt "list has no BOUND column"                          "BOUND"                    "$(bb list | head -1)"
bb delete-session "$A" >/dev/null 2>&1
is  "delete then list immediately is clean"              ""                         "$(row "$A")"
...
is   "list-tabs attributes every tab to one session" "" \
     "$(bb list-tabs | awk -v b="$B" -v c="$C" '$2==b && $2==c')"
...
is "a bad verb blames the verb, not the slug"   "unknown verb"     "$(bb "$C" teleport 2>&1)"
...
is "after detach it hangs, returning nothing" "" "$(drive "$G" 'async page => page.url()')"
bb resume "$G" >/dev/null 2>&1; sleep 5
is "bb resume does NOT revive a poisoned slug" "" "$(drive "$G" 'async page => page.url()')"
