# demonlock — architecture & internals

> **This is the technical/internals reference** (daemon/agent split, the cdhash‑authenticated socket,
> the security proofs, file layout). For **how to use** demonlock — commands, the policy language,
> the subsystems, sample outputs — see the user guide: **[README.md](README.md)**.
...
`delayzones --status`/`--abort` (view/cancel a zones change
...
demonlock delay-set-policy --status # what's queued + when it lands
