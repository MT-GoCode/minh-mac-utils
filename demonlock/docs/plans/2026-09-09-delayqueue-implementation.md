...
- [ ] Clear the inbox before the reinstall: `rm -f "/Library/Application Support/Demonlock/rv/"*` —
  old-CLI markers lack the trailing newline the new NDJSON reader requires (an unterminated single
  line reads as a torn write and is discarded); anything wanted should be re-issued with the new CLI.
  Same for the sidecar's inbox.
...
- CODE-review reconciliation (2026-09-09, reviewers on the built branch): reviewer-1 #6 questioned
  extending differential validation to the policy/gate-policy doc queues + CLI gates — KEPT
  deliberately: it entered the spec lineage via the v4.2→v4.3 regression pass (finding 3: "the CLI
  still gates absolutely… the two validators disagree"), and the flagged widening (queuing a doc
  that references an already-dangling zone) evaluates 3-valued-unknown, which can only make the
  gate STRICTER, never open it. All other reviewer-1 findings folded in code.
