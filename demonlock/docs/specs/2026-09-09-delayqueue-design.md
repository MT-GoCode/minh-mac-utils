...
**Date:** 2026-09-09 · **Status:** v2 — post-adversarial-review draft
...
- **Requeue rule.** Same key + identical payload (after canonicalisation:
  JSON re-encoded sorted-keys, unpretty) → ignored, clock kept. Same key +
...
- **Abort** by key or all. Key = the whole trimmed line of the abort marker.
  **Empty abort payload means all** **[AR#4]** — every shipped `--abort`
  writes a zero-byte marker; without this rule each becomes a silent no-op
  after upgrade while the user believes their queued loosening is cancelled.
  Aborts and replaces are always accepted, even at the cap **[AR#13]**.
...
- **Crash safety: save-before-apply for `.drop`** **[AR#3]**. The row is
  removed and `QState` saved BEFORE `apply` runs; a crash in between loses
  the request (fail-closed) instead of re-applying a loosening (fail-open —
  under v1's apply-then-save, a crash after a lockbox unlock applied would
  re-open a fresh copy window on a secret at next boot, unrequested).
  `.retry` keeps remove-after-success, which is what retry means.
- **Daemon-stamped clocks.** `requestedAt`/`applyAt` set at consumption; not
  ...
  clock moves BACKWARD past a row's `requestedAt`, the row is re-stamped
  `now + delay` (fail-closed) and logged; forward jumps are indistinguishable
...
- **Audit.** Every queue / replace / abort / apply / reject / failure appends
  ...
  keeps the last 8 outcomes per queue and status prints them
...
- **Status.** All pending rows (key, preview, lands-in), `lastAppliedAt`,
  `recent`, and `queue full (64/64)` when capped. Every status surface and
...
- **Writers:** one `write()` to `open(O_WRONLY|O_CREAT|O_APPEND|O_NOFOLLOW)`,
  under `flock(LOCK_EX)`.
- **Daemon:** `MarkerIO.consume` takes `flock(LOCK_EX)` around read+unlink
  (after the fstat owner check, so the lock is never taken on a foreign
  inode). Closes the append-between-read-and-unlink loss race.
...
**Landing is batched, not per-op [AR#2].** All due zone ops fold, in seq
order, into an in-memory copy of the live zone list; validation runs ONCE
against the *resulting* list (policy + gate-policy references all resolve;
no duplicate names); then one atomic write. Per-op validation would make
every policy-referenced zone uneditable: edit = del+add, the del alone
rejects ("referenced by policy"), then the add alone collides — both drop,
36h wasted, nothing changed. Batch-fold also removes the transient window
where the zone doesn't exist and the policy evaluates false → lockout.
If the folded result fails validation, the offending op (first by seq that
breaks it) is dropped + logged and the fold retries without it.

Individual op rules within the fold: `add` of an existing name → reject that
op; `del` of an absent name → no-op drop.
...
## Cross-queue tick order [AR#8]

Explicit and fixed: **zones → policy → gate-policy → safe-apps → presets →
lockbox**. Zones first because policies reference zones: a user who queues
`add:cafe` then a policy naming "cafe" gets both landing in the same tick in
a working order; today's order (policy first) would drop the policy
fail-closed and cost another 36h. A test asserts this pair.
...
- migration: every legacy shape; snooze-presets/lockbox sibling fields
  ...
  fail-closed.
- clock: backward jump re-stamps; forward jump lands.
